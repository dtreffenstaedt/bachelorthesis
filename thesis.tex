...
\documentclass[abstract,toc,los,lof,english,10pt,glossary,acronyms,lotl]{jluthesis}
...
\begin{figure}[H]
	...
	\caption[Current overview of detector stations]{Current overview of detector stations\cite{muonpi}. Red are future stations, green active stations.}
...
\end{figure}
\todo{Fix subfigure numbering}
When recording all coincidence times of one detector pair the different times can be plotted in a histogram. Figure \ref{fig:histogram-data}. Due to the close distance between both detectors, a large peak with Gaussian distribution can be seen which is significantly higher than the background which comprises of random coincidences due to noise in the \acrshort{sipm}. In the figure an offset can be observed which is due to hardware limitations in the detector setup.
Comparing this plot to one where both detectors are much further apart as in the case of figure \ref{fig:histogram-data} with $131\,\text{m}$, a few things are noteworthy.
...
\end{figure}
\todo{Fix citation for distance plot}
...
An approach to analyse real-time data for a network of low-cost scintillation detectors is presented and implementation details are discussed.
