...
\section{Detectors}
\subsection{Station}
Each detector station consists of a Scintillator with \acrshort{sipm} assembly, a readout board and Raspberry Pi single-board computer.
\todo{photo of Detector assembly}
\subsection{Network}
...
The Station log source is a part of the program which listens for log messages from every registered detector station. Those log messages contain a multitude of information, some of which are shown in table \ref{tab:station_logs}. Note that this is only an excerpt of the most important information and by no means complete.
\begin{table}[H]
	\centering
	\begin{tabular}{| c | c |}
		\hline
...
\end{table}
The Event source has a similar function as the Station log source but listens for all event messages from the stations. Each message consists of $8$ fields, one of which is part of the \verb*|mqtt| message topic and $7$ are part of the payload. An example message is shown in listing \ref{lst:mqtt-event}, the fields are described in table \ref{tab:event-message}.
\begin{lstfloat}[H]
	\centering
	\begin{verbatim}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.4\linewidth}
		...
		\vspace{-0.5cm}
		\begin{tikzpicture}
			\draw[ultra thick] (0,0) -- ++(1,0) node [above right] {Detector 1};
			\draw[ultra thick] (5,4) -- ++(-1,0) node [left] {Detector 2};
			...
			\node[right] at (4.5,2) {$80\,\text{cm}$};
			\node[above] at (2.25,4.9) {$80\,\text{cm}$};
		\end{tikzpicture}
	\end{minipage}
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\begin{axis}[area style, xlabel={$\Delta{t}\,[\text{ns}]$},ylabel={$n$}, xmin=-700,xmax=700]% coordinates
...
For one the total number of detected events is much lower. \todo{explanation for lower coincidence number in distance}
Secondly, \todo{plot for coincidence rate with distance}
...
The analysis program uses a data driven pipeline design for efficient execution. It supervises itself and periodically stores runtime data like CPU and memory usage in a database in order to be able to later review resource usage. This is useful in extrapolating the runtime impact for larger number of detector stations in the future.
