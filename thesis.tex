...
\caption{Example of incoming and outgoing event rate over $4\,\text{hour}$ period}
...
At this point in time the application only does the filtering step and discards the raw data. A comparison with simultaneously running post-processing analysis software has shown that the real-time application runs reliable and finds all coincidences. In fact the reverse effect can be observed since the post-processing analysis needs to read and write large amounts to and from a database on execution it creates a substantial load on the system. This leads to data being lost during that time interval since the system cannot handle those loads. The real-time system has no large load spikes so does not loose data the same way. As a backup the raw data is still stored through other means however those have never been used or necessary thus far.
