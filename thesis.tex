...
An overview of the program layout for the real-time analysis is shown in Figure \ref{fig:programlayout}, the parts Station supervision, Coincidence filter and Coincidence refinement are described in detail in their respective sections.
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}[->]
...
\subsection{Station supervision}
The station supervision tracks all currently active detector stations and classifies them for reliable and unreliable data states.
...
\begin{figure}[ht!]
	...
	\begin{tikzpicture}[->]
		...
		\node (decide1) [below=of predproc1, draw, decision] {$\sigma\geq\mu$};
