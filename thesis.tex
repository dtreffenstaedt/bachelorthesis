...
In order to determine coincidences, each event needs to be checked against all other events which are currently in the event buffer. If the coincidence coindition described before is met, the event is combined to a coincidence event and processed further. However there are cases where conflicting data has to be processed, which is shown in figure \ref{fig:conflicts}.
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
		...
		\draw (s3) -- (s4);
	\end{tikzpicture}
	\caption{Conflicting data state of four single events}
	\label{fig:conflicts}
\end{figure}
There, \tikz \draw (0,0) -- (0.5,0); marks a physical time difference and \tikz \draw[dashed] (0,0) -- (0.5,0); marks an unphysical time difference. This leads to a conflict, since all stations $s_{1,2,3,4}$ have at least one physical coincidence time to at least one other station, but also do not fit together with other stations. This means that the whole coincidence is not physically viable, though an unknown number of partial coincidences can be. This conflict required further investigation and can not be resolved easily without access to more information. At this point in time the event will be accepted but marked as containing a conflict. In the future with more data available, it might be possible to resolve such a conflict though.

\todo{plausability score}
