...
\documentclass[abstract,toc,los,lof,english,glossary,acronyms]{jluthesis}
...
\author[Daniel Treffenstädt]{Daniel Treffenstädt}
\matnr{6067797}
...
\makebeginning{
	Extensive Air Showers produced by ultra high energy cosmic radiation are still an active area of research. Due to the low rate of such events collecting statistics requires a lot of time. Different approaches to collecting data about these events can be taken, one approach is the usage of a distributed network of low-cost scintillation detectors which can be operated by persons outside the university environment to cover a maximum area without the need for large investments.
	
	In this thesis, a method is developed to perform real-time analysis for a network of low-cost plastic scintillator detectors. This network consists of small scintillator detectors with silicon Photomultiplier units which are attached to a Raspberry-Pi mini computer. Those detection units are distributed across multiple countries though the largest number is located throughout Germany. They send their data to a server where it is continuously monitored for coincidences.
...
Luftschauer, welche durch ultrahochenergetische kosmische Strahlung erzeugt werden sind ein aktives Forschungsthema. Durch die sehr geringe Rate solcher Schauer muss zum Sammeln einer ausreichend großer Datenmenge sehr viel Zeit aufgebracht werden. Die Daten können mithilfe verschiedener Methoden aufgezeichnet werden, eine ist der Einsatz eines verteilten Netzwerkes aus günstigen Szintillationsdetektoren. Diese Detektoren können aufgrund ihres niedrigen Preises und gut verfügbarer Bauteile auch durch Personen außerhalb des universitären Umfeldes betrieben werden. Das hat zum Vorteil, dass mit vergleichbar geringem finanziellen Aufwand große Flächen abgedeckt werden können. \\


In dieser Thesis wird eine Methode vorgestellt, mit welcher die Daten von solch einem Netzwerk aus Plastik-Szintillationsdetektoren in Echtzeit verarbeitet und analysiert werden können. Jeder der Detektoren besteht dabei aus einem Plastik-Szintillator mit angeschlossenem Silicon-Photomultiplier welche durch eine dedizierte Ausleseelektronik an einem Raspberry-Pi Minicomputer angeschlossen werden. Diese Detektoreinheiten werden über mehrere Länder von Privatpersonen betrieben, die größte Zahl befindet sich jedoch in Deutschland. Die Daten werden an einen zentralen Server gesendet, wo sie auf Koinzidenzen hin untersucht und abgelegt werden.


Ein Ansatz zur Rekonstruktion des primären Vertex des ultrahochenergetischen Teilchens wird vorgestellt.
\end{otherlanguage}
\clearpage
...
These are supermassive black holes at the centre of galaxies which feature an accretion disk. Within those disks, a neutral particle can be on a decaying orbit which ends close to the Schwarzschild radius where it is ionised by the effective electric charge of the black hole which in turn is induced by the magnetic fields and spin of the black hole. The now positively charged particle gets accelerated to the highest energies and follows a spiral trajectory along the spin axis while the negative particle continues on the decaying trajectory.\cite{Tursunov_2020}
\begin{figure}[ht!]
	\centering
...
\end{figure} \\
Neutral Pions $\pi^0$ have two main decay modes; one is to two photons $\gamma$, the other to an electron positron pair and a photon. With probabilities of roughly $98.823$ and $1.174\,\%$\cite[1213]{10.1093/ptep/ptaa104}, the decay cross section for the $2\gamma$ production is much higher than that of the $e^\pm + \gamma$ production. All other decay modes have orders of magnitude lower probabilities.
\begin{equation}
	\begin{aligned}
		\ce{\pi^0 &-> 2\gamma} \\
		\ce{\pi^0 &-> e^+ + e^- + \gamma} \\
...
\begin{equation}\label{eq:piondecay}
	...
	\end{aligned}
\end{equation}
The resulting secondary particles in the shower can be divided into three different categories which are shown in figure \ref{fig:shower-components}. 
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
...
The shower impact is dependent on the type of primary particle and its energy. Showers produced by \acrshort{uhecr} are classified as \acrfull{eas} and feature a larger footprint area as well as a higher particle multiplicity on earth.

A different interaction of \acrshort{uhecr} with the atmosphere is the Cherenkov radiation. Due to the highly relativistic energies, the particles travel faster than the local speed of light. In order to calculate the velocity of the particle, the relationship in equation \ref{eq:lorentz-betagamma} can be used and solved for $\beta=\frac{v}{c_0}$.
...
In the example of the \textit{Oh-My-God} particle with an energy of $3.2\cdot10^{20}\,\text{eV}$ which is assumed to have been a proton with a rest energy of $\approx938.272\,\text{MeV}$, this calculates to
\begin{equation}
...
\acrshort{uhecr} can be detected using three main methodologies.\\
Two methods depend on the cosmic air showers described in the section \ref{subsec:airshowers}, namely the muonic and the electromagnetic component. The other method relies on the Cherenkov radiation where UV telescopes can be used to detect the UV light directly. By observing the same source through multiple locations, the origin can be reconstructed.
A popular example of a particle being detected through these means is the so called \textit{Oh-My-God} particle, likely a proton or heavier nucleus with an energy of  $3.2\pm0.9\cdot10^{20}\,\text{eV}$\cite{Bird_1995} of energy. This particular particle is interesting since it was the first particle to exceed the \acrshort{gzkl}. It has been detected by the HiRes detector in Utah.
...
\end{figure}
Another method is to detect the muonic component, this can be done through scintillation detectors. Due to the muon being a charged particle, it interacts with matter and deposits energy according to figure \ref{fig:bethe-bloch}. The expected amount of deposited energy for a charged particle in matter can be described by the Bethe-Bloch formalism for $0.1\leq\beta\gamma\leq10^3$. Notably the deposited energy mainly depends on the relativistic speed of the particle described by $\beta\gamma$ where $\beta$ is the ratio of the velocity and speed of light and $\gamma$ the Lorentz factor. Using $\beta\gamma$ is especially useful since it can be easily calculated through the ratio between kinetic and rest energy. Figure \ref{fig:bethe-bloch} shows the mass stopping power of positive muons in copper.


Certain materials can convert the deposited energy to photons, a process which is known as scintillation. Figure \ref{fig:scintillator} shows the mechanism through which scintillation occurs.
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}[->]
...
\end{figure}
First, an electron is excited from the valence band into the conduction band through energy deposit from an incident particle. This electron is then transported within the conduction band to where it can drop back down to the valence band through traps. Were the electron to drop back directly, a photon would be emitted which could immediately excite another electron in the valence band. The defining features of scintillators is the existence of traps which reduce the energy of the emitted photons. Those photons then do not have enough energy to excite electrons in the material so they can pass through it. With the use of a photomultiplier, the emitted photons can then be detected. One type of Photomultiplier is the \acrfull{sipm}, which is an array of photodiodes operating above the reverse-bias break-down voltage, resulting in an avalanche upon interaction with even a single photon. The multiplicity of avalanches then is related to the number of photons interacting with the photomultiplier.

\section{Detectors}
\subsection{Station}
Each detector station consists of a Scintillator with \acrshort{sipm} assembly, a readout board and a Raspberry Pi single-board computer and is called a \acrfull{cdu}. The detectors are developed by the MuonPi project\cite{muonpi}.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	...
	\end{tikzpicture}
	\caption{Detector station assembly without Raspberry Pi computer\cite{muonpi}.}
	\label{fig:station_assembly}
\end{figure}
In figure \ref{fig:station_assembly} each component excluding the Raspberry Pi is shown. The SiPM is still detached from the Scintillator in this image since otherwise it would not be visible as distinct part. The preamplifier is part of the SiPM assembly and prepares the signal for the readout board in order to prevent loss of signal over the transmission line. The readout board supports two input channels which can be linked logically either per AND or XOR, though by default only one channel is used.

The readout board uses a u-blox NEO GNSS\cite{ublox-website} receiver chip which features an interrupt input to accurately timestamp input signals with a maximum resolution of $\approx20\,\text{ns}$ \cite{ublox-datasheet} with a dead-time of $100\,\text{ms}$.
This dead-time makes it unsuitable for high frequency applications, but due to the scintillator size of $286.5\,\text{cm}^2$ and an integral intensity of muons over the whole energy spectrum above $1\,\text{GeV}$ of $1\,\text{cm}^{-2}\text{min}^{-1}$\cite[512]{10.1093/ptep/ptaa104} the expected maximum detection rate of $4.7\,\text{Hz}$ is within the capabilities of this setup. \\
\clearpage
The detector software collects various information in addition to the timestamp with each event and sends it to the server via the \acrfull{mqtt} protocol. This is a text-based protocol providing reliable information transfer with minimal overhead\cite{mqtt}. Additional to the event information the station sends log information at regular intervals which report the current state of the detector station.
\subsection{Data exchange format}
\paragraph{Event messages} contain information about the event with additional details about the current state of the detector station. Each message consists of eight fields, one of which is part of the \acrshort{mqtt} message topic and seven are part of the payload. An example is shown in listing \ref{lst:mqtt-event}, the fields are described in table \ref{tab:event-message}. The first two fields are separated by a / character, the others are separated by spaces.
\begin{lstfloat}[H]
	\centering
	\begin{verbatim}
...
\paragraph{Log messages} contain various information about the current state of the station and are formatted as shown in Listing \ref{lst:mqtt-log}, the unit field is not present in all log messages. A small excerpt of various log message identifiers is shown in Table \ref{tab:station_logs}.
\begin{lstfloat}[H]
	\centering
	\begin{verbatim}
...
As the station in the detector network are not at known locations but distributed across multiple cities and countries at different user locations, in order to make attempts at particle reconstruction, the location information is one of the more important pieces of information from the log messages.
Additionally to this, the current quality of data is judged according to the various accuracy measurement values provided in the log messages. This is done through different approached explained in a later section.
...
\begin{figure}[H]
	...
	\begin{subfigure}{\linewidth}
		...
		\end{tikzpicture}
	\end{subfigure}
	\caption{Coincidence time histograms for various distances between mutual detector pairs.}
...
In the medium distance plot, a still significant signal is visible, though the signal-to-noise ratio is much lower. The lower number of entries is due to the shorter total runtime of this particular detector pair, which was roughly a quarter of the other two plots with about two weeks of data.

The furthest distance plot shows no significant signal over the random background.

This highlights the relationship between coincidence rate and detector distance as shown in figure \ref{fig:pair-count-rate}. There, a number of experiments have previously been performed by the MuonPi project\cite{muonpi} in order to determine the coincidence rates for different distances. A linear fit has been applied to the experimental data.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{data/pair-count-rate}
	\caption{Relationship between detector pair distance and coincidence rate.}
	\label{fig:pair-count-rate}
\end{figure}


\section{Real-time analysis}\label{sec:analysis}
In order to detect coincidences from all incoming single data points, each data point has to be compared with all others to find all possible combinations. To be able to do this a useful criterion for a coincidence has to be found. A simple approach is to select a fixed maximum time difference that two events can have in order to be counted towards a coincidence. This, however, is inefficient and leads to large numbers of false positives, since a fixed interval cannot perform well for detector combinations with large and short distances at the same time. Therefore a variable distance based criterion is used. For this purpose, the geodetic coordinates of both detector stations are taken and the straight line distance is calculated through \acrfull{enu} coordinates with the WGS84 model. The \acrshort{enu} coordinate system is a right handed local tangent plane Cartesian coordinate system $\left(\hat{e}_{East},\hat{e}_{North},\hat{e}_{Up}\right)$ which is used to set different coordinates in relation to each other. The WGS84 model describes the shape of earth as an ellipsoid and provides reference measurements for this shape\cite{wgs94}.
\begin{figure}[ht!]
	\centering
	\includegraphics[width=0.7\linewidth]{data/cr_fig5_SeaLevelMuSpectra_09}
	\caption[Muon energy spectrum from vertical and inclined muons at sea level\cite{10.1093/ptep/ptaa104}]{Muon energy spectrum from vertical \tikz \fill (0,0) rectangle(0.125,0.125); and inclined \tikz \draw[rotate=45] (0,0) rectangle(0.125,0.125); muons at sea level \cite[512]{10.1093/ptep/ptaa104}.}
	\label{fig:muon-energy-spectrum}
\end{figure} \\
With this distance the time of flight for the speed of light is calculated in order to receive the maximum physically viable time difference for one detector pair. However, this value has large uncertainties especially for detector pairs which are close together.

Since there is a constant timing offset of a few nanoseconds for each detector pair which arises through the specific hardware setup, a minimum coincidence window must be defined to capture coincidences even for pairs with a large offset. A practical value of $150\,\text{ns}$ was chosen.
It is also necessary to define an upper window, since the range of muons in air is limited and detector pairs with extremely large distances would create an abnormal and non-physical number of coincidences if left unconstrained. In order to find a useful upper limit for the coincidence window, the maximum possible flight distance for atmospheric muons has to be determined. For that, the expected maximum range of muons in air is calculated. In order to do this, a maximum expected energy must be determined through the muon energy spectrum at sea level. Figure \ref{fig:muon-energy-spectrum} shows the muon energy spectrum at sea level for two different inclinations. A cutoff value of $10\,\text{TeV}$ is selected to calculate the maximum expected distance in air. Assuming the \acrfull{csda} range for a $10\,\text{TeV}$ muon taken from \cite{muon-range} of $7.634\cdot10^5\,\frac{\text{g}}{\text{cm}^2}$ and assuming standard condition air density of $1.225\,\frac{\text{kg}}{\text{m}^3}$, the maximum expected range of muons at sea level is $62.3\,\text{km}$. That equates to a time of flight of $207.87\,\mu\text{s}$ and thus a coincidence window of larger than that is not useful.
\subsection{Coincidence and plausibility determination}\label{sec:coinc-calc}
...
\end{equation}
In equation \ref{eq:coinc_score} $t_1$ and $t_2$ denote the timestamps of both events and $t_{tof}$ the time of flight between both stations. This results in a number $c\leq1$ where a value $\leq0$ describes a non-physical coincidence.
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
	...
	\end{tikzpicture}
	\caption{Visualisation of four single events with possible coincidences in an array of four detectors $s_{1,2,3,4}$ and timestamps $t_{1,2,3,4}$.}
	\label{fig:conflicts}
\end{figure} \\
...
The goal in resolving conflicting data is to maximise $_wp$ and approach $p=1$. Without access to more data is is impossible to definitively resolve this conflict, however an approximation can be performed.
...
\end{equation}
in the graph and recalculating $_wp$ as well as $p$. This is repeated until $p=1$ is reached and $_wp$ reaches a maximum value. Should this prove to be impossible without removing too many edges for the purpose of preserving potentially useful data, the process is cancelled and the whole event is accepted tentatively.
...
\end{equation*}
can be derived. With those, the polar coordinates $(\phi, r)$ can be calculated to
...
Figure \ref{fig:reconstruction_example} shows an example for possible combinations of $r$ and $\phi$ as calculated above for a detector pair with distance of $3.4\,\text{km}$ and a coincidence time of $7.37\,\mu\text{s}$. Following the curve, the free parameter $\alpha$ is varied. The minimum possible distance is reached when $\alpha=\frac{\pi}{2}$, since then the centre of the constructed circle in figure \ref{fig:angle-assumptions} lies directly on the line between both detectors. As $\alpha$ approaches $0$, the possible radius approaches infinity and $\phi$ assumes a stationary value $\phi_\infty$.
\begin{figure}[ht!]
	\centering
...
\begin{equation*}
	...
	\end{aligned}
\end{equation*}
Noteworthy is that the possible values for $\alpha$ are constrained by a maximum value for $\delta$ due to a maximum shower footprint on earth. Adjusting for that, a lower limit for the radius r can be calculated iteratively through
...
\end{figure}\\
Figure \ref{fig:reconstruction_example_ground} shows this construction for a coincidence $n=3$, where the reference detector is located in the centre and both other detectors are located towards the right. The real world distance between the reference station and station1 is $3.4\,\text{km}$, between the reference and station2 $2\,\text{km}$.
It can be seen that the curves overlap in the bottom half of the plot, indicating a possible origin region for the primary interaction. Due to the fact that the space of possible origin points is a rotational body, the cross section between both curves is a three-dimensional manifold.
\clearpage
\section{Online coincidence processing program layout}
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[->]
		...
		\node (criterium) [left=of filter, draw, predproc,minimum height=1cm] {Coincidence criterion};
...
The station log and event sources are abstract classes which can be implemented through multiple different protocols, though at the moment only a \acrshort{mqtt} implementation is used. The same is true for the Even Sink class, though there are three distinct implementations available. The first is a sink for use with an influxDB\footnote{InfluxDB is a time-series database optimised for multiple data sources and large data streams.\cite{influxdb}} database which is used per default. Additionally there is a debugging sink which uses text output onto the console and the third is a sink to \acrshort{mqtt} which can be used if the application should be used as an instance for a local arrangement for a few detectors. This local instance then sends the processed data along to the next instance, effectively performing a preprocessing step for dense local detector arrangements.
...
\end{figure}
Figure \ref{fig:station_reliability} shows the decision process by which means each station is determined to be reliable. Each event is analysed and the mean event rate as well as the standard deviation of the rate is calculated continuously. If $\sigma\geq{k}\cdot\mu$ is true, where $\sigma$ is the standard deviation of the rate, $\mu$ the mean rate and $k$ a constant, the detector station is determined to have a highly unstable rate and is excluded from contributing events to the analysis.

Two additional factors can determine the reliability of a station, those factors are normalised so that a value of higher than 1 causes an unreliability mark.
...
\end{equation*}
where $dop$ is the dilution of precision, $h_{acc}$ the horizontal and $v_{acc}$ the vertical accuracy of the \acrshort{gnss} receiver. All those values are also determined in each detector station for itself.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[->]
		...
		\node (terminal2) [right=of predproc1, draw, terminal] {pass to coincidence filter};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[->]
		...
		\node (decide2) [below=of process1, draw, decision] {criterion met};
		\node (decide3) [right=of process1, draw, decision] {queue empty};
		\node (process2) [below=of decide2, draw, process] {add constructor to queue};
		\node (process3) [below right=of decide3, draw, process] {create new constructor};
		\node (process4) [below=of decide3, draw, process] {combine all constructors};
		\node (process5) [below=of process4, draw, process] {add event to constructor};
		
		\node (terminal2) [below=of process5, draw, terminal] {END};
...
This thesis described the possibility and implementation of a real-time system to analyse data from a distributed network of low-cost scintillation detectors. This approach offers the benefit of being able to pre-filter all incoming data for a first plausibility determination. With this, not every data point needs to be saved and the infrastructure has a lower total load.

At this point in time the application only does the filtering step and discards the raw data. A comparison with simultaneously running post-processing analysis software has shown that the real-time application runs reliably and finds all coincidences. In fact it can be observed that the real-time system detects a larger number of coincidences since the post-processing analysis needs to read and write large amounts to and from a database on execution it creates a substantial load on the system. This leads to data being lost during that time interval since the system cannot handle those loads. The real-time system has no large load spikes so does not lose data in the same way. As a backup the raw data is still stored through other means however those have not been necessary so far.

Additional benefits of a real-time system are of visual nature. It allows the real-time visualisation of found coincidences on a map through a website and so can give a visual aid as to the validity of recorded data\cite{muonpi}.
...
The methods described in section \ref{sec:analysis} need to be tested rigorously. They have undergone a basic analysis and have shown to work on a basic level though the reliability is not clear as of yet.
