...
The shower impact is dependent on the type of primary particle and its energy. In the case of \acrshort{uhecr}, the showers are classified as \acrfull{eas}. A picture of an air shower produced by a primary proton is shown in figure \ref{fig:proton-shower}. This image is produced at the KIT with the simulation toolkit CORSIKA.
\begin{figure}[ht!]
	\centering
...
The production of $\mu^\pm$ happens through the decay of $\pi^\pm$, the interaction is shown in equation \ref{eq:piondecay}.
...
Firstly, the Cherenkov radiation can be observed from the surface of earth. This light is light in the near UV spectrum and occurs since the local speed of light in the visible spectrum is lower than its vacuum speed. Due to the high energy of the primary particle its speed is nearly $1c_0$. This causes an effect comparable to a mach cone for supersonic objects in air and is known as Cherenkow radiation. This light can be detected through optical and near UV telescopes and other detectors. The first particle exceeding the \acrshort{gzkl} - the so called Oh-My-God particle - has been detected through its Cherenkov radiation by the HiRes detector in Utah. \\
Also detectable is the electromagnetic component of the showers, since its spectrum has a peak in the radio range. At those frequencies, the absorption cross section in air is low which allows them to penetrate deeply into the atmosphere. Traditional radio antennae can be used to detect the radio emissions from the shower. \\
...
$105.6583755\,\si{MeV/c^2}$. It also only has a mean lifetime of $\snsi{2.1969811}{-6}{s}$. Muons only interact weakly with matter and thus penetrate deeply into the atmosphere. This fact has been used in the past as an imaging technology called myon tomography with which the density of large masses such as volcanoes and pyramids can be recorded. This technology has famously been used to find a hidden chamber in the Cheops pyramid.
